...
TEST(Memory, GetBitstreamFromFileThrowsExceptionIfFileNotFound) {
  ...
  try {
    std::tie(dummy, dummy_s) = Emulator::Memory::get_bytestream_from_file(bad_path);
...
TEST(Memory, GetBitstreamFromFileGivesExpectedSize) {
  ...
  void *dummy;
  size_t size;
  std::tie(dummy, size) = Emulator::Memory::get_bytestream_from_file(rom_path);
...
TEST(Memory, GetBitstreamFromFileGivesExpectedContent) {
  ...
  void *content;
  size_t size;
  std::tie(content, size) = Emulator::Memory::get_bytestream_from_file(rom_path);
...
TEST(Memory, ConvertBitstreamToVectorGivesExpectedSize) {
  ...
  converted_stream = Emulator::Memory::convert_bytestream_to_vector(rom_stream, rom_size);
